...
process.narrowpeak <- function(narrow.file, chr.size, half.width=NULL, summit="offset", stop.exclusive=T, broadpeak=F, gappedpeak=F, peakranger=F){


  aa <- read.table(narrow.file, comment.char="#")

  if(broadpeak){
    bb.ori <- data.frame(chr=aa$V1, start=aa$V2, stop=aa$V3, signal.value=aa$V7, p.value=aa$V8, q.value=aa$V9)
  }else if(gappedpeak){
    bb.ori <- data.frame(chr=aa$V1, start=aa$V2, stop=aa$V3, signal.value=aa$V13, p.value=aa$V14, q.value=aa$V15)
  }else if(peakranger){
      bb.ori <- data.frame(chr=aa$V1, start=aa$V2, stop=aa$V3, signal.value=-125*log2(aa$V5), p.value=-log10(aa$V5), q.value=-log10(aa$V5))
  }else{
